...
struct EthereumBlock<B> {
	...
	pub cached_contract_address: Option<Option<Address>>,
}
...
impl<B: Blockchain> EthereumBlock<B> {
	...
	fn read_contract_address(&mut self) -> Option<Address> {
		if let Some(contract_address) = self.cached_contract_address {
		...
		self.cached_contract_address = Some(contract_address);
		contract_address
	}
}
